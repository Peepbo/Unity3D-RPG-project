...
namespace KevinIglesias {
    ...
    public class IKHelperTool : MonoBehaviour {
		...
		public void StartSequence(int id, IKType goal, List<IKSequence> iKSequence, bool smoothEntry, bool clearOnExit)
		{
			if(!CheckAvailability(id))
			{
				return;
			...
			StopIK(id);
			state.isActive = true;
			state.stateCoroutine = PerformSequence(id, iKSequence, smoothEntry, clearOnExit);
			StartCoroutine(state.stateCoroutine);
		}
